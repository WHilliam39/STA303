...
Black Saber Software (BSS), a technology company, has no data collection on its current and new staff members' race or ethnicity. In order to optimize the electronic technology interaction (EDI) and test fairness, this study assesses every three dimensions in hiring, salary, and promotion based on the existing employees and hiring data from BSS.

We estimate that, in each hiring, salary and promotion dimension, applicants' ability and their technical test ratings are more influential than other factors in the recruitment process; salary and promotion, however, indicate a gap between males and females.

The results of the study are summarized below.

- For Hiring Process:
  - The hiring initial application phase has no bias on race, but only GPA, previous work experience, and extracurricular.
  - The technical task, hiring phase two, mainly concentrated on applicants' writing, speaking, technical skill and leadership presence scores.
  - Final hiring data shows a gender preference in newly hired employees with respect to the proportion of males and females. However, in the interview section, recruitment phase three, indicates that none of the factors, in terms of gender, interview rating, would affect the final hiring results; namely, no factor bias.
- For Salary:
  - In 2021, there are 607 employees with annual wages in a range of 25,000 dollars to 154,200 dollars per year in Blakc Saber. 
  - When men and women have the same employment characteristics and job functions in Black Saber, women earn 2259 dollars less than an equivalent man annually. The group employees with gender(prefer not to say) are also likely to earn less than men with weak evidence.
  - The group of employees who need improvements at their levels are benefited with around 245 dollars per year than the people who are qualified or exceed the expectation of their levels.
- For Promotion:
  - The males have occupied a larger percentage that more than half in all the nine senior positions, the Vice President with the largest percentage in males, which is 70%.
  - The number of promotions would decrease by 0.2943 times on average when the employee is female rather than male, it illustrates females have a relatively smaller number of promotions than the males, though they have satisfied the requirements of leadership.

\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/productivity_salary_ggplot.png}
  \caption{This figure is representative of the spread of salary with the scale of productivity (grouped by gender), which looks at the salary for all men, women, prefer not to say regardless of job type or employment characteristics. The men are more likely to receive the highest wage, which includes a potential tendency that more productivity comes with a lower pay.}
\end{figure}

\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/promotion_productivity.png}
  \caption{Gender Distribution on productivity by number of promotion.}
\end{figure}
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.8\textwidth]{./images/hiring_interview.png}
  \caption{Interview Ratings from Phase 3}
\end{figure}

According to \textit{Figure 4}, although there are fewer females in phase 3 but their rating are greater than or equal to most males. From this figure we could not get helpful information because the number of females is too small and there could be possible bias in rating when the interviews were taken.   
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.8\textwidth]{./images/hiring_gpa.png}
  \caption{GPA Distribution}
\end{figure}


From \textit{Figure 5}, all candidates that were finally hired have a GPA over 3.0. And males' GPA seems higher than females'. This seems that the hiring process is based on candidates' ability.
...
```
\textit{Figure 6} to \textit{Figure 9} visualize the abilities of candidates in different genders, which involved technical skills, writing skills, speaking skills, and leadership.

\begin{figure}[H]
\centering
    \includegraphics[width=0.9\textwidth]{./images/hiring_techskill.png}
    \caption{Technical Skills Distribution.}
\end{figure}

- From \textit{Figure 6}, we can see that for all genders the plot is approximately normally distributed, which makes sense because most of the people are within the median level and those whole are really good at or bad at something are rare. In this case, the gender ratio in next phase should be around 1 to 1. So from this figure there could be some unfair in this process. 

\begin{figure}[H]
\centering
    \includegraphics[width=0.9\textwidth]{./images/hiring_writingskill.png}
    \caption{Writing Skills Distribution.}
\end{figure}

- From \textit{Figure 7}, we can see that although it seems approximately normally distributed for all genders but scores of males are higher then females. Therefore, it makes sense that more males passed this phase. 

\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/hiring_speakingskill.png}
  \caption{Speaking Skills DIstribution.}
\end{figure}

\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/hiring_leadership.png}
  \caption{Leadership Distribution.}
\end{figure}

- From \textit{Figure 8} and \textit{Figure 9}, there are more males with higher ratings. This also supports the finding from the previous one. 
...
For phase two we have parameters in \textit{Table 2}. Like the first model,  we use the result of phase 2 as the response variable and fitted a logistic model.
\begin{table}[H]
\centering
...
current_employee %>% ggplot(aes(x=productivity, fill=gender)) + geom_histogram(binwidth=3) + theme_minimal() +     
  labs(title="Histogram of Productivity, Black Saber Software",
      x="Value of Productivity",
      y="Frequencies",
      caption=str_c("Created by: White Sheild Analytics\n","Source: Black Saber Software\n")) +
  scale_fill_brewer(palette = "Paired")
...
current_employee %>% ggplot(aes(x=productivity, y=gender, color=gender)) + geom_boxplot() + theme_minimal() + labs(title="Box Plot of Productivity Grouped by Gender, Black Saber Software",
      x="Productivity",
      y="Gender",
      caption=str_c("Created by: White Sheild Analytics\n","Source: Black Saber Software\n")) + scale_color_brewer(palette = "Paired")
ggsave("images/productivity_box.png",width = 7, height = 4)

current_employee %>% ggplot(aes(x=salary, y=gender, color=gender)) + geom_boxplot() + theme_minimal() + labs(title="Box Plot of Salaries Grouped by Gender, Black Saber Software",
      x="Salary",
      y="Gender",
      caption=str_c("Created by: White Sheild Analytics\n","Source: Black Saber Software\n")) + scale_color_brewer(palette = "Paired")
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./images/productivity_histo.png}
    \caption{Histgram of Frequencies by Productivity.}
\end{figure}
    
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./images/salary_histo.png}
    \caption{Histgram of Frequencies by Salary.}
\end{figure}
...
Recall the research question that we want to analyze the fairness of the salary process, so we created a basic histogram \textit{Figure 10} to have a look at the frequencies of records on different productivity values and salary numbers. The productivity is close to a symmetric bell curve shape which may imply a normal distribution of productivity of employees. But for the salary, the histogram \textit{Figure 11} indicates an accumulation of records around 40,000 dollars per year and there are two small groups of people with higher than usual salaries in the range of 120,000 dollars to 160,000 dollars per year. These extreme high annual pays may come from those employees as executives and senior managers. So in this stage we can raise a concern about the unbalanced relation between productivity and salary. Then we calculate the correlation between the salary and productivity since we want to see how strong the salary and productivity are related in this firm. The value we get is - 0.0356, so productivity is negatively correlated to the salary in a weak correlation, which means that when the value of productivity rises but the salary decreases. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./images/productivity_box.png}
    \caption{Box plot of productivity grouped by gender.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./images/salary_box.png}
    \caption{Box plot of salary grouped by gender.}
\end{figure}
...
\centering
  \includegraphics[width=0.9\textwidth]{./images/productivity_salary_ggplot.png}
  \caption{Scatter plot of productivity by salary.}
\end{figure}

After having a look at the big picture, we want to focus on the gender parity in salary processing. In the \textit{Figure 12} and \textit{Figure 13} which are box plots of productivity and salary grouped into woman, Prefer not to say and the man. The man has a comparably lower productivity median but higher salary median, and salary from three groups are positively skewed which also indicates a group of records with high salaries. This is caused by the minimum wage by law and also the natural construction of an organization and company. In the \textit{Figure 14} which is a scatter plot of productivity by salary, we still can see there are three major stages that salaries accumulated, the number of women and men seems proportional to the ratio of gender in the company, but also we can see the man occupied the highest salaries on every stage of salaries with a wide range of productivity value.
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/gender_role_senority.png}
  \caption{Gender Ratio in Role Senority.}
\end{figure}

- From \textit{Figure 15}, we can see that the females group concentrates on one or two times of promotion, though she has relatively larger productivity than the males. The distribution of the males is more concentrated in the center of productivity and a larger number of promotion. 
- From\textit{Figure 16}, it illustrates that the males group has more chance to promoted than females because the men has occupied a larger percentage when the position increases.
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/promotion_exceeds_leadship.png}
  \caption{Gender Distribution on exceeded leadership by the number of promotion.}
\end{figure}
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.9\textwidth]{./images/gender_leadership.png}
  \caption{Gender Ratio in Leadership Level.}
\end{figure}

- From \textit{Figure 17}, we can see that the majority of employees' leadership exceed their expectation are males.

- From \textit{Figure 18}, we can see that the gender in "leadership exceed expectation" category is only men, and the female has occupied above 90% in "leadership needs to improve" category.
...
By using the likelihood ratio test table \textit{Table 8}, we can find that the likelihood ratio Chi-Squared statistic and the p-value is smaller than 0.05, which is 0.0178, it represents the average productivity is an important effect when the employees have the chance to promote. However, from the AIC and BIC value, the model without productivity effect has a relatively smaller value, it represents this model perform better on fitting this data set. After combine the two model results, the female group perhaps has few number of promotion and less opportunity to promote than the male, though the there is not exist an obvious different about productivity in the male and female groups from the \textit{Figure 15}.
...
Over the fairness assessments in hiring, salary, and promotion dimensions of Black Saber Software, few conclusions have been drawn. Concerning the hiring process, fairness analysis has been taken into the company's overall recruitment process. In the initial application, GPA, internship experiment, and extracurriculars are essential factors taking into account. In phase 2, technical task, only the skill ratings are significantly related to the pass of the second phase outcome, not gender. In phase three, in fact, neither the race nor interviewer rating has an influence on final hiring. The recruitment process is most likely concentrated on applicants' abilities and experience. When it comes to salary, the gender gap is prominent, where male's average wage is 2259 dollars annually more than females. We also noticed that employees who need improvement are likely to have high salaries. Moreover, gender has a crucial factor in internal promotion, where males occupied a relatively large percentage in all nine senior positions. 
